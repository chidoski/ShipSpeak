-- ShipSpeak Database Schema
-- Designed for PostgreSQL (Supabase)
-- Version: 1.0 - Epic 1: System Integration & Data Persistence
...
CREATE INDEX idx_profiles_role ON profiles(current_role);
...
-- Analysis indexes
CREATE INDEX idx_meeting_analyses_meeting_id ON meeting_analyses(meeting_id);
CREATE INDEX idx_meeting_transcripts_meeting_id ON meeting_transcripts(meeting_id);
CREATE INDEX idx_meeting_transcripts_was_analyzed ON meeting_transcripts(was_analyzed);

-- Scenario indexes
CREATE INDEX idx_scenario_templates_category ON scenario_templates(category);
CREATE INDEX idx_scenario_templates_difficulty ON scenario_templates(difficulty_level);
CREATE INDEX idx_generated_scenarios_user_id ON generated_scenarios(user_id);
CREATE INDEX idx_generated_scenarios_meeting_id ON generated_scenarios(meeting_id);
